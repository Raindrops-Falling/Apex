...
export function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          {['Product', 'Resources'].map(item => (
            <button key={item} className="flex items-center gap-1 text-sm" style={{ color: 'rgba(28,40,20,0.65)' }}>
        ...
        <div className="hidden md:flex items-center gap-2.5">
          {session ? (
            <>
              <Link to="/upload"
                className="text-sm px-5 py-2 rounded-full flex items-center gap-2 no-underline"
            ...
            <>
              <Link to="/login"
                className="text-sm px-5 py-2 rounded-full no-underline"
                style={{ border: '1px solid rgba(28,40,20,0.25)', color: '#1c2814' }}>
                Login
              </Link>
              <Link to="/upload"
      ...
      {menuOpen && (
        <div className="md:hidden px-6 py-5 flex flex-col gap-5 border-t"
            ...
            {session ? (
              <>
                <Link to="/upload" onClick={() => setMenuOpen(false)}
                  className="text-sm py-2.5 rounded-full flex items-center justify-center gap-2 no-underline"
                  style={{ backgroundColor: '#1c2814', color: '#f5f0e8' }}>
                  Dashboard <ArrowRight size={13} />
                </Link>
                <button onClick={handleLogout}
              ...
              <>
                <Link to="/login" onClick={() => setMenuOpen(false)}
                  className="text-sm py-2.5 rounded-full text-center no-underline"
                  style={{ border: '1px solid rgba(28,40,20,0.25)', color: '#1c2814' }}>
                  Login
                </Link>
                <Link to="/upload" onClick={() => setMenuOpen(false)}
